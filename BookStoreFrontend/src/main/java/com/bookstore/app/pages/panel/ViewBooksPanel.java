...
import com.bookstore.app.pages.Checkout;
...
public class ViewBooksPanel extends Panel {
	...
	private void init() {
		...
		DropDownChoice categories = new CategoryDropDownChoice("categories",
				new PropertyModel(dataProvider, "category"), categoryTOs, new ChoiceRenderer("name", "id"),
				bookDataView);
		...
		form.add(new Button("addToCart") {
			public void onSubmit() {
				...
				setResponsePage(new Checkout(ViewBooksPanel.this.booksMarkedForCheckout));
			}
		});
	...
	class BookDataView extends DataView {
		...
		protected void populateItem(Item item) {
			...
			item.add(new Label("price", book.getPrice().toString()));
	...
	class CategoryDropDownChoice extends DropDownChoice {
		...
		public CategoryDropDownChoice(String id, IModel model, List displayData, IChoiceRenderer renderer,
				DataView bookDataView) {
